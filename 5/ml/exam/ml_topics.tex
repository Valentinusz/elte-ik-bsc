...
\begin{document}
	...
	Autoencoders are neural networks trained to reproduce their input data at the output layer. By using a bottleneck layer in the middle, autoencoders can learn a compressed representation of the input data, which can be used for generating new samples.
	...
	\begin{itemize}
		...
		\end{itemize}
		\item From their current location if not carrying any: pick up a piece of food
		\item If at nest an carrying food: put down food
		\item Ants deposite a unit of pheromone at their current location
		\begin{itemize}
	...
	\begin{itemize}
		\item a graph is used, pheromone is placed on the edges
		...
		\item when most ants select the same solution the simulation stops
	\end{itemize}
	...
	\begin{itemize}
		...
		\item each agent has a personal tolerance level (in percentage)
		\item an agent is happy if its tolerance level is higher than the share of its neighbours from the other group
		\item if an agent is not happy it moves to an empty tile
	\end{itemize} 
	...
	The algorithm is trained on a labeled dataset, where each output is known. The goal is to create a function that predicts the output of new unlabeled data.
	
	
	\subsection{Unsupervised learning}
	The algorithm is trained on a dataset without any labels. The goal is to create a function that labels data.
	...
	Reinforcement learning involves an agent that learns to make decisions by interacting with an environment. The agent receives feedback in the form of rewards or penalties based on the actions it takes, allowing it to learn optimal strategies over time.
	...
	A decision tree is used to model the mapping between input vectors and decisions (output labels). It a sequence of tests done starting from the root until a leaf node is reached.
	
	The simplest kind of decision tree is a boolean decision tree, which are actually just logical statements of the form:
	\[
	...
	It is important to notice this decision tree has shorter paths and is much simpler than our original tree. The learning algorithm looks at the examples, not at the actual function (because that function is what we want to determine through the learning process).
	
	This means that the built tree is fitted to our training data. This results in decision tress not generalizing well:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Scalability to large data sets
		\item Versatility in handling discrete and continuous inputs
		\item Performing classification and regression
	\end{itemize}
	
	\subsection{Cons}
	\begin{itemize}
		\item Suboptimal accuracy (largely due to the greedy search)
		\item If trees are very deep, then getting a prediction for a new example can be expensive 
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item recalculate the center points (add and take the average of sample vectors)
	...
	\begin{multicols}{2}		
		...
		\end{figure}
	\end{multicols}
	After this the center points need to be recalculated:
	\begin{align*}
	...
	Reinforcement learning is a type of machine learning paradigm where an agent learns to make decisions by interacting with an environment in order to maximize some notion of cumulative reward over time.
	\\[8pt]
	\noindent
	...
	There are many ways to select chromosomes to survive to the next generation. In roulette wheel selection the chance of selecting an individual is proportionate to its fitness value.
	
	\subsection{Crossover}
	A random locus is selected. The tails of the individuals are exchanged.
	\begin{figure}[H]
		\centering
	...
	Each evolutionary operator is defined a bit differently. The whole approach requires the definition of:
	
	\begin{itemize}
		\item set of terminals
	...
	\begin{itemize}
		...
		\item particles are firefly who emit light
		\item brightness depends on how good of a solution they provide
		\item light intensity reduces over distance and respects absorption
		\item each individual moves towards brighter fireflies
	\end{itemize}
	
	Attractiveness is calculated as $(I_0, d, \gamma) = I_0 \cdot e^{-\gamma d^2}$, where $d$ is a distance metric and $\gamma$ is the absorption coefficient.
	
	All fireflies approach better solutions and also do some random movement.
	...
	Population based meta-heuristics algorithm simulating the leadership hierarchy and hunting mechanism of grey wolves in nature. The species adhere to a strict social structure. In our case:
	
	\begin{itemize}
		\item $\alpha$ best solution
		\item $\beta$ second best solution
		\item $\delta$ third best solution
		\item $\omega$ rest of the solutions
	\end{itemize}
	
	\noindent
	Finding the optimal solution is like hunting prey. Hunting is guided by the 3 best solutions, $\omega$ wolves follow:
	\begin{itemize}
		\item find a fit prey (exploration)
		\item encircle it
		\item attack it (exploitation)
	\end{itemize}
	...
	A single neuron is unable to solve complex problem, however an interconnected system of neurons can deliver complex behaviour. $\varphi$ is the activation function responsible for keeping the values in given range.
